...
public class Longset_Sub
{
 		public static void main(String args[])
			throws IOException
		{
			int arr[] = { 1, 9, 3, 10,11,12,4, 8,2,7,5 };//After saving into PQ the values are sorted as : {1,2,3,4,5,7,8,9,10,11,12}
			int n = arr.length;
			System.out.println(
	...
	static int findLongestConseqSubseq(int arr[], int N)
	{
		...
		for (int i = 1; i < N; i++)
		{
			/*check if current peek element minus previous element is greater then 1 This is done because 
			if it's greater than 1 then the sequence doesn't start or is broken here */
			if (pq.peek() - prev > 1)
			{
			...
			{
				// Update the counter,These are consecutive elements
				c++;
